...
export function ExcluirOrgBtn({ orgId, nome }: { orgId: string; nome: string }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await excluirOrganizacao(orgId, password)
      if (result?.error) {
        ...
        >
          <div className="w-full max-w-sm rounded-xl border border-border bg-card p-6 shadow-xl space-y-4 mx-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-foreground">Excluir organização</p>
